"""
Annotation agent prompts for scinr-ingest.
...
RULE 2 — CONSERVATIVE MATCHING:
  If no catalog model covers >= 50% of the node's actual content, set
  ...
  Never select a model because it is the "closest" when coverage is below 50%.
...
STEP 5 — Decision — select best match or null:
  (a) If the best candidate covers >= 50% of the node's actual content:
      -> matched_model_class = that model's exact CatalogModel enum key (CamelCase)
      -> confidence = "high" if coverage >= 75%, "medium" if 50-74%, "low" if < 50%
   ...
   (b) If no candidate covers >= 50% of the node's actual content:
...
STEP 7 — Produce the AnnotationDecision output:
  ...
  - confidence: "high" (>= 75%) / "medium" (50-74%) / "low" (< 50% or null)
...
5. NEVER produce an empty reasoning field — it must reference specific content
   from the node.
6. NEVER force-fit a model when coverage is below 50% — use null instead.
